...
if(gameState===3){
    ...
    text("Press A to start the practice again!!!!", width/2, 350);
     }
...
function spawncoin(){
if(frameCount%100===0){
    var coin = createSprite(width,height-100)
    coin.addImage("a",coinImg);
    coin.y=Math.round(random(height-200,height-400));
    coin.velocityX=-4;
    coin.scale=0.2
    coinGroup.add(coin);
}
...
function spawnDanger(){
    if(frameCount%200===0){
        var danger = createSprite(width,height-20)
        danger.addImage("a",dangerImg);
        danger.y=Math.round(random(height-100,height-200));
        danger.velocityX=-4;
        danger.scale=1
